...
def load_swear_counts():
    return pd.read_csv("data/front_page/counts.csv", index_col=0)

@st.cache_data(ttl=None, max_entries=4, show_spinner=False)
def load_swear_counts_byyear():
    return pd.read_csv("data/front_page/counts_byyear.csv", index_col=0)

@st.cache_data(ttl=None, max_entries=4, show_spinner=False)
def load_raw_swear_counts_byyear():
    return pd.read_csv("data/front_page/rawnumber_byyear.csv", index_col=0)
...
with open("data/front_page/data_front_page.txt", "r") as f:
    lines = f.readlines()
...
with open("data/front_page/swears_bydecade.json", "r") as f:
